...
if($lastpage > 1) { 
    ...
    if ($page > 1) {
        $pagination.= "<li class='page-item'><a class='page-link' href=\"$targetpage&page=$prev\"><</a></li>"; 
    }
    ...
    if ($lastpage < 7 + ($adjacents * 2)) { 
        for ($counter = 1; $counter <= $lastpage; $counter++) {
            if ($counter == $page) {
                ...
                $pagination.= "<li class='page-item page-link active'>$counter</li>";
             } else {
        ...
        if($page < 1 + ($adjacents * 2)) {
            for ($counter = 1; $counter < 4 + ($adjacents * 2); $counter++) {
                if ($counter == $page) {
                    //$pagination.= "<li class='page-item'><a class='page-link' href='#' class='active'>$counter</a></li>";
                    $pagination.= "<li class='page-item'>$counter</li>";
                } else {
                    $pagination.= "<li class='page-item'><a class='page-link' href=\"$targetpage&page=$counter\">$counter</a></li>"; 
                }
            }
    ...
    //next button
    if ($page < $counter - 1) {
        $pagination.= "<li class='page-item'><a class='page-link' href=\"$targetpage&page=$next\">></a></li>";
    } else {
        $pagination.= "";
        $pagination.= "</ul></div>\n"; 
    }    
